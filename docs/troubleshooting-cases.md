...
Root cause:
Firewall rules blocked DNS traffic from VM network to host virbr0.
...
Problem:
rTorrent was slow and port status was not correct.
...
torrent client port must match provider forwarded port
...
Root cause:
deluge-console was executed without correct user/config path and failed authentication.
...
speedtest inside VPN container
checked Gluetun logs
checked /tmp/gluetun/forwarded_port

Root cause:
Endpoint did not provide working NAT-PMP/port forwarding even though VPN tunnel worked.
...
VPN public IP and port were verified
...
basic Kubernetes objects: namespace, deployment, pod, service
